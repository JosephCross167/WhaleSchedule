...
const chrome = spawn(CHROME, ["--headless=new", "--remote-debugging-port=9390",
  `--user-data-dir=${join(ROOT, "_tmp_verify", "chrome-profile-sw-k")}`,
...
for (let i = 0; i < 100; i++) { try { if ((await fetch("http://127.0.0.1:9390/json/version")).ok) break; } catch {} await new Promise((r) => setTimeout(r, 250)); }
const tgt = await (await fetch("http://127.0.0.1:9390/json/new?about:blank", { method: "PUT" })).json();
